...
def graph_group_level(children: List[Dict[str, Any]], masks: Dict[str, np.ndarray], rgb: np.ndarray, fields: Dict[str, np.ndarray], entity_type: str, level: int, config: Dict[str, Any], threshold_multiplier: float) -> List[Dict[str, Any]]:
    ...
    used: set[str] = set(); grouped: List[List[Dict[str, Any]]] = []
    ...
    for edge in sorted(edges, key=lambda item: (-item["mergeAffinity"], item["sourceId"], item["targetId"])):
        ...
        if merged_mask.sum() > max_area or not connected(merged_mask):
            continue
        used.update((source_id, target_id)); grouped.append(pair)
    grouped.extend([[child] for child in children if child["id"] not in used])
    ...
    for group in grouped:
        ...
        for child in group:
            mask |= masks[child["id"]]
        parent = make_entity(parent_id, entity_type, level, 1, mask, rgb, fields, child_ids, {"operation": "merge" if len(group) > 1 else "carry", "parents": child_ids})
        for child in group:
