...
def bounded_residual(original: np.ndarray, base: np.ndarray, config: Dict[str, Any]) -> Tuple[np.ndarray, np.ndarray, bytes | None, Dict[str, Any]]:
    ...
    candidate_indices = np.flatnonzero((magnitude.ravel() > 0) & np.any(quantized_full.reshape(-1, 3) != 0, axis=1))
    ...
    if candidate_limit < candidate_indices.size:
        candidate_magnitudes = magnitude.ravel()[candidate_indices]
        chosen = np.argpartition(candidate_magnitudes, -candidate_limit)[-candidate_limit:]
        candidate_indices = candidate_indices[chosen]
    candidate_indices = candidate_indices[np.argsort(-magnitude.ravel()[candidate_indices], kind="stable")]
    ...
    metadata = {
        "schema": "QuantizedSparseResidual@0.6", "quantizationStep": step, "budgetBytes": budget, "actualEncodedBytes": encoded_bytes,
        ...
        "candidatePixels": int(candidate_indices.size), "coveredPixels": kept, "coverage": rounded(kept / max(magnitude.size, 1), 8), "selection": "largest_rgb_l1_quantized_residual", "heuristicRateDistortion": rate_distortion(mse, encoded_bytes, magnitude.size, config),
    }
