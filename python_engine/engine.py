...
def analyze(input_path: Path, output_dir: Path, raw_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    rd_summary = {"constant": rate_distortion(constant_metrics["mse"], len(micro_regions) * 12, height * width, config), "parametric": rate_distortion(parametric_metrics["mse"], model_bytes, height * width, config), "residual": residual_metadata["rd"]}
    ...
    experiment = {"id": f"exp-{config_hash(config)}-{int(started * 1000)}", "engineVersion": SCHEMA_VERSION, "configHash": config_hash(config), "algorithm": "graph_agglomerative+adaptive_reconstruction", "timestampEpochMs": int(time.time() * 1000)}
    representation: Dict[str, Any] = {
        ...
        "graph_metadata": {"construction": "region adjacency plus spatial/appearance KNN candidates; graph agglomeration uses merge affinity", "relationshipDensity": rounded(len(relationships) / max(len(all_entities), 1), 8), "candidateSources": ["adjacent", "spatial_knn", "appearance_knn", "hierarchy"]}, "segmentationDiagnostics": segmentation_diagnostics,
        ...
        "reconstruction_metadata": {"outputs": reconstruction_metadata, "rateDistortion": rd_summary, "residual": residual_metadata, "errorArtifacts": {"absolutePixelError": "errors/absolute-error.png", "parametricError": "errors/parametric-error.png", "perRegionError": "errors/per-region-error.png", "residualEnergy": "errors/residual-energy.png"}}, "scale_correspondence": {"method": "Hungarian minimum cost on IoU, normalized centroid, appearance, and log-area terms", "links": cross_scale}, "scale_consistency": correspondence_summary, "validity": validity, "profiling": profile,
