...
While an advanced sensitivity study is active, the workbench shows a compact **ETA range**, not a deadline. The range is bounded by the remaining advanced-process budget and improves from a conservative budget estimate to observed-progress or sensitivity-variant progress as server events arrive. A completed private result records an ordered **Execution timeline** with server-observed durations for validation, analysis stages, artifact upload, and finalization. These values include bridge/orchestration work and are not benchmark results or performance guarantees.
