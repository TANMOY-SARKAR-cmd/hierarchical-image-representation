...
Upload admission validates canonical base64 plus consistent filename extension, MIME type, and binary signature for PNG, JPEG, or WebP **before** accepting an asynchronous job. It enforces byte and pixel ceilings, temporary-workspace cleanup, a 120-second normal child-process limit, a bounded no-progress watchdog, fixed-window per-owner submission limits, and a process-local concurrent-job cap. A requested capped sensitivity study receives a longer but finite workload-aware process budget; its nested deterministic variants forward `sensitivity` progress heartbeats so legitimate computation remains observable. The durable owner-scoped progress snapshot coalesces repeated same-stage heartbeats into one timing entry and exposes a bounded ETA **range** only for active advanced studies. Completed private results serialize `AnalysisExecutionTiming@1` with ordered stage durations and total server-observed orchestration time. Completed result payloads can be restored after process-local cache eviction until their configured expiry; aggregate-only cache telemetry is administrator-only.
