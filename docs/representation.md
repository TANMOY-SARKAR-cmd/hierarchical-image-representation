...
Upload admission validates canonical base64 plus consistent filename extension, MIME type, and binary signature for PNG, JPEG, or WebP **before** accepting an asynchronous job. It enforces byte and pixel ceilings, temporary-workspace cleanup, a 120-second normal child-process allowance, a bounded no-progress watchdog, fixed-window per-owner submission limits, and a process-local concurrent-job cap. A normal job may earn a fixed 30-second extension only at an active `merge_tree` budget boundary when a recent public-safe merge heartbeat exists; its total normal allowance is capped at five minutes. This policy never extends stale work, non-merge stages, or the separate advanced-study budget, and it never alters segmentation, scale, reconstruction, residual, or sensitivity fidelity. A requested capped sensitivity study receives a longer but finite workload-aware process budget; its nested deterministic variants forward `sensitivity` progress heartbeats so legitimate computation remains observable. The durable owner-scoped progress snapshot coalesces repeated same-stage heartbeats into one timing entry, retains at most eight distinct public-safe messages per stage with non-negative stage-relative offsets, records only bounded allowance totals, and exposes a bounded ETA **range** only for active advanced studies. The workbench renders server percentage as factual completion and its separate ETA band only as a client-side visualization of that range. Completed private results serialize `AnalysisExecutionTiming@1` with ordered stage durations, bounded stage-message detail, and total server-observed orchestration time. Completed result payloads can be restored after process-local cache eviction until their configured expiry; aggregate-only cache telemetry is administrator-only.

The global energy merge tree also emits rate-limited `merge_tree` heartbeats while evaluating candidate relationships and deterministic energy scores. These events preserve the bounded true-stall watchdog while preventing active hierarchy work from being misclassified as silent. A recent heartbeat can prove that hierarchy work is active at a normal-budget boundary, allowing only the bounded standard extension described above. They do not reduce segmentation detail, alter the accepted merge order, disclose image content, grant an unbounded runtime, or extend the finite advanced processing budget.

The CLI emits a public-safe `initializing_engine` event before importing heavyweight engine dependencies. Node treats that event as a bounded cold-start phase, never longer than the finite total process allowance; it then restores the ordinary no-progress deadline once validation begins. Failed and completed workbench statuses freeze their elapsed display at the server terminal timestamp, so an old failure card does not imply that the child process continued to execute.

When a browser-private job fails, its private status may include `AnalysisFailure@1`: an opaque diagnostic token plus a bounded category and last safe stage. This explains whether the failure reached child spawn, startup, ordinary stage liveness, finite budget, engine exit, or completion parsing without exposing raw engine diagnostics or image-derived data. A terminal receipt is immutable for that job lifecycle; later timeout, close, or cancellation events cannot rewrite it.

Users may cancel an in-flight analysis through the workbench, including the visible sensitivity phase. A cancellation terminates the active Python child process, closes the in-progress timing entry, records a terminal `cancelled` state, and never exposes a partial result. The server records privacy-safe correlation and stage logs, and converts a no-progress child process into a safe terminal failure. A retryable terminal failure offers a same-settings retry while the local source file remains selected. Users may also discard a completed analysis from the workbench. Discarding or expiry removes cached result and inspection evidence, timing snapshots, and the manifest payload/workbench artifact references. The current storage interface does not expose physical object deletion, so these are access revocations rather than assertions of immediate backend byte erasure.
