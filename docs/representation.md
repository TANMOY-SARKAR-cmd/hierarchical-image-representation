...
Upload admission validates canonical base64 plus consistent filename extension, MIME type, and binary signature for PNG, JPEG, or WebP **before** accepting an asynchronous job. It enforces byte and pixel ceilings, temporary-workspace cleanup, a 120-second normal child-process limit, a bounded no-progress watchdog, fixed-window per-owner submission limits, and a process-local concurrent-job cap. A requested capped sensitivity study receives a longer but finite workload-aware process budget; its nested deterministic variants forward `sensitivity` progress heartbeats so legitimate computation remains observable. The durable owner-scoped progress snapshot coalesces repeated same-stage heartbeats into one timing entry, retains at most eight distinct public-safe messages per stage with non-negative stage-relative offsets, and exposes a bounded ETA **range** only for active advanced studies. The workbench renders server percentage as factual completion and its separate ETA band only as a client-side visualization of that range. Completed private results serialize `AnalysisExecutionTiming@1` with ordered stage durations, bounded stage-message detail, and total server-observed orchestration time. Completed result payloads can be restored after process-local cache eviction until their configured expiry; aggregate-only cache telemetry is administrator-only.

The global energy merge tree also emits rate-limited `merge_tree` heartbeats while evaluating candidate relationships and deterministic energy scores. These events preserve the bounded true-stall watchdog while preventing active hierarchy work from being misclassified as silent. They do not reduce segmentation detail, alter the accepted merge order, disclose image content, or extend the finite normal or advanced processing budgets.
