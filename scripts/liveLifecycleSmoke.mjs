...
const summary = { mode: runSensitivity ? "five_variant_sensitivity" : "primary", terminal: "", elapsedMs: 0, observedStatuses: [], observedStages: [], advancedEtaObserved: false, startupHeartbeatObserved: false, mergeTreeHeartbeatObserved: false, verification: [], failedAt: "startup" };
...
try {
  ...
  const startupStage = terminal.job.timing?.stages?.find(stage => stage.stage === "initializing_engine");
  if (!startupStage || (startupStage.messages?.length ?? 0) < 1) {
    throw new Error("The completed analysis did not retain engine-startup progress detail.");
  }
  summary.startupHeartbeatObserved = true;
  summary.verification.push("engine-startup progress detail");
