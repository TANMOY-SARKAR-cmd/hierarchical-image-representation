...
export default function Home() {
  ...
  const finalErrorHeatmapUrl = artifacts?.errors.absolutePixelError ?? null;
        ...
        <section className="min-w-0 space-y-4">
          <section className="overflow-hidden rounded-xl border border-cyan-100/10 bg-slate-900/80 shadow-2xl shadow-slate-950/20">
            ...
            <div className="grid gap-px bg-white/10 md:grid-cols-2">
              ...
              <div className="min-h-72 bg-slate-950 p-3"><div className="mb-2 flex items-center justify-between font-mono text-[10px] uppercase tracking-[0.16em] text-slate-500"><span>Reconstructed output</span><span className="text-emerald-300">{reconstructionLevel === "full" ? "MICRO-REGION MEAN" : reconstructionLevel.toUpperCase()}</span></div><div className="mb-2 rounded border border-amber-300/15 bg-amber-300/[0.035] p-2"><div className="flex items-center justify-between gap-2"><Button type="button" variant="outline" size="sm" disabled={!finalErrorHeatmapUrl} onClick={() => { if (!finalErrorHeatmapUrl) return; if (!showErrorHeatmap) setReconstructionLevel("residual"); setShowErrorHeatmap(value => !value); }} className={cn("h-6 border-amber-300/20 px-2 font-mono text-[9px]", showErrorHeatmap ? "bg-amber-300/15 text-amber-100" : "text-slate-500")}>ERROR HEATMAP {showErrorHeatmap ? "ON" : "OFF"}</Button><span className="font-mono text-[9px] text-amber-100">{finalErrorHeatmapUrl ? `${Math.round(errorHeatmapOpacity * 100)}%` : "UNAVAILABLE"}</span></div><div className={cn("mt-2", (!showErrorHeatmap || !finalErrorHeatmapUrl) && "opacity-40")}><input type="range" aria-label="Error heatmap opacity" min="0" max="100" value={Math.round(errorHeatmapOpacity * 100)} onChange={event => setErrorHeatmapOpacity(Number(event.target.value) / 100)} disabled={!showErrorHeatmap || !finalErrorHeatmapUrl} className="h-1.5 w-full cursor-pointer accent-amber-300" /></div><p className="mt-2 text-[10px] leading-relaxed text-slate-500">Brighter areas show greater server-measured absolute RGB difference between the input and final reconstruction.</p></div>{reconstructionUrl ? <div className="relative overflow-hidden rounded border border-white/8 bg-black"><img src={reconstructionUrl} alt="Hierarchically reconstructed image" className="max-h-[480px] w-full object-contain" />{showErrorHeatmap && finalErrorHeatmapUrl ? <img src={finalErrorHeatmapUrl} alt="Absolute reconstruction error heatmap overlay" className="pointer-events-none absolute inset-0 h-full w-full object-contain" style={{ opacity: errorHeatmapOpacity }} /> : null}</div> : <div className="relative grid h-72 place-items-center overflow-hidden rounded border border-dashed border-emerald-300/15 bg-[radial-gradient(circle_at_50%_50%,rgba(16,185,129,0.07),transparent_42%)] text-center"><div className="absolute grid h-32 w-44 grid-cols-4 gap-1 opacity-35">{Array.from({ length: 16 }).map((_, index) => <span key={index} className="rounded-sm border border-emerald-300/30" style={{ transform: `translate(${(index % 3) - 1}px, ${Math.floor(index / 4) % 2 ? 2 : -2}px)` }} />)}</div><div className="relative"><Network className="mx-auto mb-3 h-8 w-8 text-emerald-500/60" /><p className="text-sm font-medium text-slate-400">Decode a region hierarchy to inspect fidelity.</p><p className="mt-1 font-mono text-[10px] uppercase tracking-wider text-slate-600">Regions → mean appearance → PNG / SVG</p></div></div>}</div>
            </div>
          </section>

          <section className="overflow-hidden rounded-xl border border-violet-200/15 bg-slate-900/80 shadow-2xl shadow-slate-950/20"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/8 px-4 py-3"><div><h2 className="text-sm font-semibold text-violet-100">Swipe comparison</h2><p className="mt-0.5 font-mono text-[10px] uppercase tracking-widest text-slate-500">Optional source-versus-reconstruction inspection</p></div>{representation ? <div className="flex items-center gap-2 font-mono text-[10px]"><span className="rounded border border-cyan-300/20 bg-cyan-300/[0.06] px-2 py-1 text-cyan-100">PSNR {Number(selectedReconstructionOutput?.psnr ?? 0).toFixed(2)} dB</span><span className="rounded border border-emerald-300/20 bg-emerald-300/[0.06] px-2 py-1 text-emerald-100">Correction {residualMetadata?.artifactEmitted ? `${(residualMetadata.coverage * 100).toFixed(1)}%` : "not applied"}</span></div> : null}</div>{sourceUrl && reconstructionUrl && representation ? <div className="bg-slate-950 p-3"><ImageComparisonSlider sourceUrl={sourceUrl} reconstructionUrl={reconstructionUrl} position={comparisonPosition} onPositionChange={setComparisonPosition} width={representation.image.width} height={representation.image.height} reconstructionLabel={reconstructionLevel === "full" ? "Structural reconstruction" : reconstructionLevel.toUpperCase()} /></div> : <div className="grid min-h-40 place-items-center bg-slate-950 p-6 text-center"><div><Network className="mx-auto mb-2 h-7 w-7 text-violet-400/60" /><p className="text-sm font-medium text-slate-400">Run an analysis to enable an independent swipe comparison.</p><p className="mt-1 font-mono text-[10px] uppercase tracking-wider text-slate-600">Original input stays separate above</p></div></div>}</section>
